...
import com.link.preview.library.DefaultImagePickingStrategy
import com.link.preview.library.DefaultUrlExtractionStrategy
import com.link.preview.library.Regex
import com.link.preview.library.SourceContent
...
data class Preview(val title: String, val description: String, val imageUrls: List<String>)
...
object LinkPreLoader {
    ...
    fun load(link: String): Preview {
        ...
        val url = if (urlStrings.isEmpty()) "" else unshortenUrl(urlStrings[0])
        ...
        if (url != "") {
            if (isImage(url) && !url.contains("dropbox")) {
                ...
                val document: Document =
                    Jsoup.connect(sourceContent.finalUrl).userAgent("Mozilla").get()
                ...
                try {
                    ...
                    if (sourceContent.title == "") {
                        val matchTitle =
                            Regex.pregMatch(sourceContent.htmlCode, Regex.TITLE_PATTERN, 2)
                        if (matchTitle != "") sourceContent.title = htmlDecode(matchTitle)
                    }

                    if (sourceContent.description == "") sourceContent.description =
                        crawlCode(sourceContent.htmlCode)

                    sourceContent.description =
                        sourceContent.description.replace(Regex.SCRIPT_PATTERN.toRegex(), "")

                    if (imagePickingStrategy.imageQuantity != NONE) {
                        val images: List<String> =
                            imagePickingStrategy.getImages(document, metaTags)
        ...
        val finalLinkSet =
            sourceContent.finalUrl.split("&".toRegex()).dropLastWhile { it.isEmpty() }
                .toTypedArray()
    ...
    private fun cannonicalPage(url: String): String {
        ...
        for (i in url.indices) if (url[i] != '/') cannonical += url[i] else break
    ...
    private fun getMetaTags(content: String): HashMap<String, String> {
        ...
        for (match in matches) {
            val lowerCase = match.toLowerCase(Locale.US)
